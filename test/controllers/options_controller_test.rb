...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference('Option.count') do
      post options_url, params: { option: { data_clean_task_id: @option.data_clean_task_id, name: @option.name } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { data_clean_task_id: @option.data_clean_task_id, name: @option.name } }
